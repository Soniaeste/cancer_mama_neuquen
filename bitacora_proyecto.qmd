...
```{r}  library(leaflet)  mapa_2010 <- ap_tiempo |>   filter(Año == 2010) |>   st_transform(4326)}
```
...
Para evitar conflictos de escritura sobre el GeoPackage previamente utilizado en el proyecto, se decidió guardar la capa espacial longitudinal destinada a la visualización en un archivo independiente.

```{r}
archivo_mapa <- "datos/procesados/ap_tiempo_mapa.gpkg"
...
Finalmente se verificó la correcta creación del archivo `ap_tiempo_mapa.gpkg`, comprobando que la capa `ap_tiempo` conserva los 224 registros correspondientes a las 28 Áreas Programáticas observadas durante los ocho años del período 2010–2017.
